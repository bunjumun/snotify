...
- **This file describes the branch you have checked out, not necessarily the live site.** As of 2026-08-14 the V2 takeover, the `vendor/` move and Phase 1b were committed on `v2-takes-over` and unpushed, while `origin/main` still served V1 at `/game/` with V2 behind its own door at `/game-v2/`. So read the table above as the shape of your working tree, and run `git log origin/main` before claiming anything about what a player currently sees. `docs/session-brief.md` pins both shapes side by side.
